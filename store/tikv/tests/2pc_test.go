...
func (s *testCommitterSuite) TestContextCancelCausingUndetermined(c *C) {
	...
	committer, err := txn.NewCommitter(0)
	c.Assert(err, IsNil)
	_ = committer.PrewriteAllMutations(context.Background())
	c.Assert(err, IsNil)
...
func (s *testCommitterSuite) TestWrittenKeysOnConflict(c *C) {
	...
	for i := 0; i < 10; i++ {
		txn1 := s.begin(c)
		txn2 := s.begin(c)
		_ = txn2.Set([]byte("x1"), []byte("1"))
		committer2, err := txn2.NewCommitter(2)
		c.Assert(err, IsNil)
		err = committer2.Execute(context.Background())
		c.Assert(err, IsNil)
		_ = txn1.Set([]byte("x1"), []byte("1"))
		_ = txn1.Set([]byte("y1"), []byte("2"))
		...
		txn3 := s.begin(c)
		start := time.Now()
		_, _ = txn3.Get(context.TODO(), []byte("y1"))
		totalTime += time.Since(start)
		_ = txn3.Commit(context.Background())
	}
...
func (s *testCommitterSuite) TestDeleteYourWriteCauseGhostPrimary(c *C) {
	...
	_, _ = txn1.Get(context.Background(), k1)
	_ = txn1.GetMemBuffer().SetWithFlags(k1, []byte{0}, kv.SetPresumeKeyNotExists)
	_ = txn1.Set(k2, []byte{1})
	_ = txn1.Set(k3, []byte{2})
	_ = txn1.Delete(k1)
...
func (s *testCommitterSuite) TestDeleteAllYourWrites(c *C) {
	...
	_ = txn1.GetMemBuffer().SetWithFlags(k1, []byte{0}, kv.SetPresumeKeyNotExists)
	_ = txn1.Delete(k1)
	_ = txn1.GetMemBuffer().SetWithFlags(k2, []byte{1}, kv.SetPresumeKeyNotExists)
	_ = txn1.Delete(k2)
	_ = txn1.GetMemBuffer().SetWithFlags(k3, []byte{2}, kv.SetPresumeKeyNotExists)
	_ = txn1.Delete(k3)
	err1 := txn1.Commit(context.Background())
	c.Assert(err1, IsNil)
}
